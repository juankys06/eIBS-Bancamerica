...
public class PdfParser {
	...
	private static long getStartXref(RandomAccessFile raf) throws
		...
		raf.seek(fileSize - PjConst.SCAN_STARTXREF);
		...
		StringBuffer sb = new StringBuffer();
		int x = 0;
		while (Character.isDigit((char)(buffer[x]))) {
			sb.append((char)(buffer[x]));
			x++;
			if (x >= buffer.length) {
				throw new
					StartxrefFormatException("Unexpected end of file (startxref).");
			}
		}
	...
	public static PjObject parse(Pdf pdf, RandomAccessFile raf, long[][] xref, byte[] data, int start)
		...
		while ( ( ! endFlag ) && (getToken(state)) ) {
			if (state._stream != -1) {
				...
				PjNumber gen = (PjNumber)(stack.pop());
				PjNumber obj = (PjNumber)(stack.pop());
				stack.push(new PjReference(obj, gen));
			}
	...
	public static void parseXref(byte[] data, long[][] xref, int start) {
		PdfParserState state = new PdfParserState();
		state._data = data;
		state._pos = start;
		getLine(state);  // throw out initial "xref"
		StringTokenizer st;
		int index, count, x;
		while (state._pos < state._data.length) {
			getLine(state);
