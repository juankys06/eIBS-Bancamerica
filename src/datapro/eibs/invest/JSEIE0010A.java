...
public class JSEIE0010A extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE001001")) {
			...
			if (IsNotError) {  // There are no errors
				...
				try {
						flexLog("About to call Page: " + LangPath + "EIE0010A_inv_port_basic.jsp");
						callPage(LangPath + "EIE0010A_inv_port_basic.jsp", req, res);	
				}
				catch (Exception e) {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EIE001101")) {
			...
			if (IsNotError) {  // There are no errors
				...
				try {
						flexLog("About to call Page: " + LangPath + "EIE0010A_inv_port_acc.jsp");
						callPage(LangPath + "EIE0010A_inv_port_acc.jsp", req, res);	
				}
				catch (Exception e) {
						flexLog("Exception calling page " + e);
				}								
	...
	protected  void procReqPortBene(
		...
		try {
			...
			if (newmessage.getFormatName().equals("ESD000004")) {
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog("About to call Page: " + LangPath + "EIE0010A_inv_port_bene.jsp");
						callPage(LangPath + "EIE0010A_inv_port_bene.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				} else { // There are errors
