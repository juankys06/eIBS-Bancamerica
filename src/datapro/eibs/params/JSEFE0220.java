...
public class JSEFE0220 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqEnterParameter(
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EFE0220_exchange_rate_enter.jsp");
			callPage(LangPath + "EFE0220_exchange_rate_enter.jsp", req, res);
		} catch (Exception e) {
	...
	protected void procActionPos(
		...
		switch (inptOPT) {
			case 1 : //New
				res.sendRedirect(
					super.srctx
						+ "/servlet/datapro.eibs.params.JSEFE0220?SCREEN=300");
				break;
			case 2 : //Maintenance
	...
	protected void procActionEnterRates(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
		...
		String bank = req.getParameter("E01RATBNK").trim();
		String currency = req.getParameter("E01RATCCY").trim();
		...
		switch (inptOPT) {
			case 1 : //List
				res.sendRedirect(
					super.srctx
						+ "/servlet/datapro.eibs.params.JSEFE0220?SCREEN=100" + "&E01RATBNK=" + bank + "&E01RATCCY=" + currency);
				break;
			case 2 : //Maintenance
			res.sendRedirect(
				super.srctx
					+ "/servlet/datapro.eibs.params.JSEFE0220?SCREEN=1100" + "&E01RATBNK=" + bank + "&E01RATCCY=" + currency);
				break;
	...
	protected void procReqRatesList(
		...
		// Send Initial data
		try {
			msgList = (EFE022001Message) mc.getMessageRecord("EFE022001");
			...
			msgList.setE01RATCCY(req.getParameter("E01RATCCY"));
			msgList.send();
			msgList.destroy();
			flexLog("EFE022001 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
			...
			if (newmessage.getFormatName().equals("EFE022001")) {
				...
				try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "EFE0220_exchange_rate_list.jsp");
					callPage(
						LangPath + "EFE0220_exchange_rate_list.jsp",
						req,
						res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
	...
	protected void procReqMaintenanceDetail(
		MessageContext mc,
		ESS0030DSMessage user,
		HttpServletRequest req,
		HttpServletResponse res,
		...
		// Send Initial data
		try {
			msgRT = (EFE022001Message) mc.getMessageRecord("EFE022001");
			msgRT.setH01USERID(user.getH01USR());
			msgRT.setH01PROGRM("EFE022001");
			msgRT.setH01TIMSYS(getTimeStamp());
			msgRT.setH01SCRCOD("01");
			msgRT.setH01OPECOD("0002");
			msgRT.setE01RATBNK(req.getParameter("E01RATBNK"));
			msgRT.setE01RATCCY(req.getParameter("E01RATCCY").toUpperCase());
			msgRT.send();
			msgRT.destroy();
			flexLog("EFE022001 Message Sent");
		} catch (Exception e) {
				...
				if (IsNotError) { // There are no errors
					try {
						flexLog(
							"About to call Page: "
								+ LangPath
								+ "EFE0220_exchange_rate_details.jsp");
						callPage(
							LangPath + "EFE0220_exchange_rate_details.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					...
					try {
						flexLog(
							"About to call Page: "
								...
								+ "EFE0220_exchange_rate_enter.jsp");
						callPage(
							LangPath + "EFE0220_exchange_rate_enter.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
		...
		try {
			...
			if (newmessage.getFormatName().equals("EFE022001")) {
				...
				if (IsNotError) { // There are no errors
					res.sendRedirect(
						super.srctx
							+ "/servlet/datapro.eibs.params.JSEFE0220?SCREEN=1");
				} else { // There are errors
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "EFE0220_exchange_rate_details.jsp");
			callPage(LangPath + "EFE0220_exchange_rate_details.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
				...
				try {
					...
					switch (screen) {
						...
						case R_NEW :
							...
							break;
						case R_MAINT_DETAIL :
							procReqMaintenanceDetail(mc, msgUser, req, res, session);
							break;	
						case R_MAINTENANCE :
						...
						case R_ENTER :
							...
							// Action
						case A_ENTER :
							procActionEnterRates(mc, msgUser, req, res, session);
							break;	
						case A_POSITION :
