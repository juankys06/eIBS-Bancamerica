...
public class JSESD0590 extends datapro.eibs.master.SuperServlet {
	...
	protected void procReqEnterBranch(
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "ESD0590_branch_enter.jsp");
			callPage(LangPath + "ESD0590_branch_enter.jsp", req, res);
		} catch (Exception e) {
	...
	protected void procActionPos(
		...
		switch (inptOPT) {
			...
			default :
				res.sendRedirect(
					super.srctx
						+ "/servlet/datapro.eibs.params.JSESD0590?SCREEN=500");
				break;
		}
	}
	...
	protected void procReqBranchList(
		...
		// Send Initial data
		try {
			msgList = (ESD059001Message) mc.getMessageRecord("ESD059001");
			...
			msgList.send();
			msgList.destroy();
			flexLog("ESD059001 Message Sent");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
		...
		try {
			...
			if (newmessage.getFormatName().equals("ESD059001")) {
				...
				while (true) {
					...
					msgList = (ESD059001Message) newmessage;
				...
				flexLog("Putting java beans into the session");
				ses.setAttribute("ESD059001Help", beanList);
				ses.setAttribute("userPO", userPO);
				...
				try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "ESD0590_branch_list.jsp");
					callPage(
						LangPath + "ESD0590_branch_list.jsp",
						req,
						res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
	...
	protected void procReqBranchListDelete(
		...
		// Send Initial data
		try {
			JBObjList bl = (JBObjList) ses.getAttribute("ESD059001Help");
			int idx = Integer.parseInt(req.getParameter("CURRCODE"));
			bl.setCurrentRow(idx);

			msgList = (ESD059001Message) bl.getRecord();
			//msgList = (ESD059001Message) mc.getMessageRecord("ESD059001");
			msgList.setH01USERID(user.getH01USR());
			msgList.setH01PROGRM("ELD010003");
			msgList.setH01TIMSYS(getTimeStamp());
		...
		try {
			...
			if (newmessage.getFormatName().equals("ELEERR")) {
				...
				if (IsNotError) {
					res.sendRedirect(super.srctx + "/servlet/datapro.eibs.params.JSESD0590?SCREEN=100");
				} else {
					try {
						flexLog("About to call Page: " + LangPath + "ESD0590_branch_list.jsp");
						callPage(LangPath + "ESD0590_branch_list.jsp", req, res);						
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
				}
	...
	protected void procActionMaintenance(
		...
		try {
			...
			msgRT = (ESD059001Message) ses.getAttribute("brnDetails");
		...
		try {
			...
			if (newmessage.getFormatName().equals("ESD059001")) {
				try {
					msgRT = new datapro.eibs.beans.ESD059001Message();
					flexLog("ESD059001 Message Received");
				} catch (Exception ex) {
					flexLog("Error: " + ex);
				}

				msgRT = (ESD059001Message) newmessage;
				...
				if (IsNotError) { // There are no errors
					res.sendRedirect(
										super.srctx
											+ "/servlet/datapro.eibs.params.JSESD0590?SCREEN=100");
					...
					try {
						...
						callPage(
							LangPath + "ESD0590_branch_maintenance.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
	...
	protected void procReqNew(
		...
		try {
			flexLog("Send Initial Data");
			msgRT = (ESD059001Message) mc.getMessageRecord("ESD059001");
			msgRT.setH01USERID(user.getH01USR());
			msgRT.setH01PROGRM("EGL036001");
			msgRT.setH01TIMSYS(getTimeStamp());
			msgRT.setH01SCRCOD("01");
			...
			msgRT.setE01BRNNUM(req.getParameter("E01BRNNUM"));
		...
		try {
			flexLog(
				"About to call Page: "
					+ LangPath
					+ "ESD0590_branch_maintenance.jsp");
			callPage(LangPath + "ESD0590_branch_maintenance.jsp", req, res);
		} catch (Exception e) {
			flexLog("Exception calling page " + e);
		}
	...
	protected void procReqMaintenance(
		...
		try {
			...
			msgDoc = (ESD059001Message) bl.getRecord();
