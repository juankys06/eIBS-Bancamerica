...
public class JSEFIX000 extends datapro.eibs.master.SuperServlet {
...
protected void procReqEnterMaintenance(
	...
	try {
		flexLog(
			"About to call Page: "
				+ LangPath
				+ "EFIX000_fix_enter_maintenance.jsp");
		callPage(LangPath + "EFIX000_fix_enter_maintenance.jsp", req, res);
	} catch (Exception e) {
...
protected void procReqNew(
	...
	msgPR = new EFIX00001Message();

	flexLog("Putting java beans into the session");
	ses.setAttribute("error", msgError);
	ses.setAttribute("fix", msgPR);
	ses.setAttribute("userPO", userPO);

	try {
		flexLog(
			"About to call Page3: " + LangPath + "EFIX000_fix_new.jsp");
		callPage(LangPath + "EFIX000_fix_new.jsp", req, res);
	} catch (Exception e) {
		flexLog("Exception calling page " + e);
	}
...
protected void procActionEnterMaintenance(
	...
	// Send Initial data
	try {
		msgRT = (EFIX00001Message) mc.getMessageRecord("EFIX00001");
		...
		try {
			msgRT.setE01FIXASN(req.getParameter("E01FIXASN"));
		} catch (Exception e) {
		}
		...
		try {
			msgRT.setE01FIXBRN(req.getParameter("E01FIXBRN"));
		} catch (Exception e) {
		}
	...
	try {
		...
		if (newmessage.getFormatName().equals("EFIX00001")) {
			try {
				msgRT = new datapro.eibs.beans.EFIX00001Message();
				flexLog("EFIX00001 Message Received");
			} catch (Exception ex) {
				flexLog("Error: " + ex);
			}

			msgRT = (EFIX00001Message) newmessage;
			...
			try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "EFIX000_fix_maintenance.jsp");
					callPage(
						LangPath + "EFIX000_fix_maintenance.jsp",
						req,
						res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				...
				try {
						flexLog(
							"About to call Page: "
								...
								+ "EFIX000_fix_enter_maintenance.jsp");
						callPage(
							LangPath + "EFIX000_fix_enter_maintenance.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
...
protected  void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		flexLog("Send Initial Data");
	 	msgInst = (EFIX00001Message)ses.getAttribute("fix");
		...
		while (enu.hasMoreElements()) {
	 	...
	 	msgInst.destroy();
	 	flexLog("EFIX00001 Message Sent");
	}		
	catch (Exception e)	{
		e.printStackTrace();
		flexLog("Error: " + e);
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				msgInst = (EFIX00001Message)newmessage;
				...
				if (IsNotError) {  // There are no errors
					try {
						    flexLog("About to call Page: " + LangPath + "EFIX000_fix_enter_maintenance.jsp");
							callPage(LangPath + "EFIX000_fix_enter_maintenance.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
						...
						try {
							flexLog("About to call Page: " + LangPath + "EFIX000_fix_maintenance.jsp");
							callPage(LangPath + "EFIX000_fix_maintenance.jsp", req, res);	
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
	    ...
	    newmessage = mc.receiveMessage();
				...
				if (IsNotError) {  // There are no errors
					try {
						...
						try {
							flexLog("About to call Page: " + LangPath + "EFIX000_fix_new.jsp");
							callPage(LangPath + "EFIX000_fix_new.jsp", req, res);	
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);
