...
public class JSEFIX030 extends datapro.eibs.master.SuperServlet {
...
protected void procReqEnterMaintenance(
	...
	try {
		flexLog(
			"About to call Page: "
				+ LangPath
				+ "EFIX030_fix_enter_cross_references.jsp");
		callPage(LangPath + "EFIX030_fix_enter_cross_references.jsp", req, res);
	} catch (Exception e) {
...
protected void procActionEnterMaintenance(
	...
	// Send Initial data
	try {
		msgRT = (EFIX03001Message) mc.getMessageRecord("EFIX03001");
		msgRT.setH01USERID(user.getH01USR());
		msgRT.setH01PROGRM("EFIX03001");
		...
		try {
			msgRT.setE01FIXCCY(req.getParameter("E01FIXCCY"));
		} catch (Exception e) {
		}

		try {
			msgRT.setE01FIXGLN(req.getParameter("E01FIXGLN"));
		} catch (Exception e) {
		}
	...
	try {
		...
		if (newmessage.getFormatName().equals("EFIX03001")) {
			try {
				msgRT = new datapro.eibs.beans.EFIX03001Message();
				flexLog("EFIX00001 Message Received");
			} catch (Exception ex) {
				flexLog("Error: " + ex);
			}

			msgRT = (EFIX03001Message) newmessage;
			...
			try {
					flexLog(
						"About to call Page: "
							+ LangPath
							+ "EFIX030_fix_cross_references.jsp");
					callPage(
						LangPath + "EFIX030_fix_cross_references.jsp",
						req,
						res);
				} catch (Exception e) {
					flexLog("Exception calling page " + e);
				...
				try {
						flexLog(
							"About to call Page: "
								...
								+ "EFIX030_fix_enter_cross_references.jsp");
						callPage(
							LangPath + "EFIX030_fix_enter_cross_references.jsp",
							req,
							res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
...
protected  void procActionMaintenance(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		flexLog("Send Initial Data");
	 	msgInst = (EFIX03001Message)ses.getAttribute("fix");
	...
	try
	{
	    newmessage = mc.receiveMessage();
				...
				msgInst = (EFIX03001Message)newmessage;
				...
				if (IsNotError) {  // There are no errors
					try {
						    flexLog("About to call Page: " + LangPath + "EFIX030_fix_enter_cross_references.jsp");
							callPage(LangPath + "EFIX030_fix_enter_cross_references.jsp", req, res);
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
						...
						try {
							flexLog("About to call Page: " + LangPath + "EFIX030_fix_cross_references.jsp");
							callPage(LangPath + "EFIX030_fix_cross_references.jsp", req, res);	
						}
						catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
