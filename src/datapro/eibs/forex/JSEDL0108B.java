...
public class JSEDL0108B extends datapro.eibs.master.SuperServlet {

	// Foreign Exchange Deals
	protected static final int A_PLP			= 4;
	protected static final int A_PLP_MOD			= 6;	
	
	protected static final int R_APPROVAL_INQ			= 200;	
...
protected void procActionEnterCommPap(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		msgDeal = (EDL010802Message)mc.getMessageRecord("EDL010802");
	 	msgDeal.setH02USERID(user.getH01USR());
	 	msgDeal.setH02PROGRM("EDL0108");
		...
		try {
		 	if (req.getParameter("E02DLSREF") != null)
		 	  msgDeal.setE02DLSREF(req.getParameter("E02DLSREF"));
		}
		catch (Exception e)	{
	 	  msgDeal.setE02DLSREF("0");
		}
		try {
		...
		try {
	 	...
	 	msgDeal.send();
	 	msgDeal.destroy();
	 	flexLog("EDL010802 Message Sent");
	}		
	catch (Exception e)	{
		e.printStackTrace();
		flexLog("Error: " + e);
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDL010802")) {
			try {
				msgDeal = new EDL010802Message();
		  	} catch (Exception ex) {
				flexLog("Error: " + ex); 
		  	}

			msgDeal = (EDL010802Message)newmessage;
			...
			if (IsNotError) {  // There are no errors
					try {
						flexLog("About to call Page: " + LangPath + "EDL0108B_fe_cp.jsp");
						callPage(LangPath + "EDL0108B_fe_cp.jsp", req, res);	
					}
					catch (Exception e) {
...
protected void procActionCommPap(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		msgDeal = (EDL010802Message)ses.getAttribute("deal");
		msgDeal.setH02USERID(user.getH01USR());
		msgDeal.setH02PROGRM("EDL0108");
		msgDeal.setH02TIMSYS(getTimeStamp());
		msgDeal.setH02SCRCOD("01");
	...
	try
	{
		...
		try {
			msgDeal = new EDL010802Message();
			flexLog("EDL010802 Message Received");
		} 
		catch (Exception ex) {
			flexLog("Error: " + ex); 
		}
		...
		if (IsNotError) {  // There are no errors
			try {
				flexLog("About to call Page: " + LangPath + "EDL0108B_fe_cp_mod.jsp");
				callPage(LangPath + "EDL0108B_fe_cp_mod.jsp", req, res);
			}
			catch (Exception e) {
				flexLog("Exception calling page " + e);
			}
...
protected void procActionPLPMod(ESS0030DSMessage user,
	...
	try {
		flexLog("About to call Page: " + LangPath + "EDL0108B_fe_cp_confirm.jsp");
		callPage(LangPath + "EDL0108B_fe_cp_confirm.jsp", req, res);
	} catch (Exception e) {
		flexLog("Exception calling page " + e);
	}
...
protected void procReqApprovalInq(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		try {
			if (req.getParameter("E02DEACUN") != null)
			  msgDeal.setE02DEACUN(req.getParameter("E02DEACUN"));
		}
		catch (Exception e)	{
		  msgDeal.setE02DEACUN("");
		}
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDL010802")) {
			...
			if (IsNotError) {  // There are no errors
					try {
						flexLog("About to call Page: " + LangPath + "EDL0108B_ap_fe_cp.jsp");
						callPage(LangPath + "EDL0108B_ap_fe_cp.jsp", req, res);	
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
					}					
