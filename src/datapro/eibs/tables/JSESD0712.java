...
public class JSESD0712 extends datapro.eibs.master.SuperServlet {
			...
			try {
				...
				try {
					...
					if (newmessage.getFormatName().equals("ELEERR")) {
						...
						if (newmessage.getFormatName().equals("ESD0712DS")) {
							try {
								msgDDAServCharge =
									(datapro.eibs.beans.ESD0712DSMessage) Beans.instantiate(
										getClass().getClassLoader(),
										"datapro.eibs.beans.ESD0712DSMessage");
							...
							// showESD0712DS(msgDDAServCharge);

							flexLog("Putting java beans into the session");
							session.setAttribute("ddaServCharge", msgDDAServCharge);

							try {
								flexLog("About to call Page: " + LangPath + "ESD0712_dda_inq_serv_charge.jsp");
								callPage(LangPath + "ESD0712_dda_inq_serv_charge.jsp", req, res);
							} catch (Exception e) {
								flexLog("Exception calling page " + e);
							}
						} else
