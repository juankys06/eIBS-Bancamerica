...
public class JSEDC0000 extends SuperServlet {
	...
	public void destroy() {
		try {
			if (s != null) s.close();
			flexLog("free resources used by JSEDC0000");
		} catch (Exception e) {
			e.printStackTrace();
			flexLog("Error: " + e);
		}
	...
	private void procActionCommissions(HttpServletRequest req, HttpServletResponse res, int screen) throws IOException {
		...
		try {
			MessageContextHandler msgHandle = new MessageContextHandler(mc);
			EDC000002Message msg = (EDC000002Message) msgHandle.initMessage("EDC000002", user.getH01USR(), "0005");
			initTransaction(String.valueOf(screen), "MAINTENANCE", "");
	...
	private void procActionOpeningMaint(HttpServletRequest req, HttpServletResponse res, int screen) throws IOException {
		...
		try {
			MessageContextHandler msgHandle = new MessageContextHandler(mc);
			EDC000001Message msg = (EDC000001Message) session.getAttribute("dcNew");
			msg = (EDC000001Message) msgHandle.initMessage(msg, user.getH01USR(), "0005");
			initTransaction(String.valueOf(screen), "MAINTENANCE", "");
			userPO.setCusNum(req.getParameter("E01DCMCUN"));
			msg.setH01SCRCOD("01");
			msgHandle.setFieldsFromPage(req, msg);
			...
			String PageToCall = "EDC0000_dc_opening.jsp";
	...
	private void procReqOpeningMaint(HttpServletRequest req, HttpServletResponse res, int screen) throws IOException {
		...
		try {
			...
			if (req.getParameter("E01DCMACC") != null || (userPO.getIdentifier() != null && !userPO.getIdentifier().equals(""))) {
				...
				MessageContextHandler msgHandle = new MessageContextHandler(mc);
				EDC000001Message msg = (EDC000001Message) msgHandle.initMessage("EDC000001", user.getH01USR(), opCode);
				msg.setH01SCRCOD("01");
				try { 
				...
				if (isNotError) {
					...
					if (!firstScreen) {
						PageToCall = "EDC0000_dc_opening.jsp";
					}
				}
	...
	private void procReqOpening(HttpServletRequest req, HttpServletResponse res, int screen) throws IOException {
		...
		try {
			MessageContextHandler msgHandle = new MessageContextHandler(mc);
			EDC000001Message msg = (EDC000001Message) msgHandle.initMessage("EDC000001", user.getH01USR(), "0001");
