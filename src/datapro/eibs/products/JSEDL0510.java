...
public class JSEDL0510 extends datapro.eibs.master.SuperServlet {
		...
		try {
			  newmessage = mc.receiveMessage();
				...
				if (newmessage.getFormatName().equals("EDL051001")) {
					...
					try {
						flexLog("About to call Page: " + LangPath + "EDL0510_ln_prod_sel_tables.jsp");
						callPage(LangPath + "EDL0510_ln_prod_sel_tables.jsp", req, res);
					} catch (Exception e) {
		...
		try {
			...
			if (newmessage.getFormatName().equals("EDL051002")) {
				...
				if (IsNotError) { // There are no errors
						...
						try {
							flexLog("About to call Page: " + LangPath + "EDL0510_ln_prod_table_maint.jsp");
							callPage(LangPath + "EDL0510_ln_prod_table_maint.jsp", req, res);
						} catch (Exception e) {
							flexLog("Exception calling page " + e);
						}
	...
	protected void procReqEnterNew(
		...
		userPO = (datapro.eibs.beans.UserPos) ses.getAttribute("userPO");
					...
					try {
						flexLog("About to call Page: " + LangPath + "EDL0510_ln_prod_enter_sel_tables.jsp");
						callPage(LangPath + "EDL0510_ln_prod_enter_sel_tables.jsp", req, res);
					} catch (Exception e) {
						flexLog("Exception calling page " + e);
					}
