...
public class JSEDL0290 extends datapro.eibs.master.SuperServlet {
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDL029001")) {
				...
				if (IsNotError) {  // There are no errors
					try {
						    flexLog("About to call Page1: " + LangPath + "EDL0290_ln_cal.jsp");
							callPage(LangPath + "EDL0290_ln_cal.jsp", req, res);
					}
					catch (Exception e) {
					...
					try {
						flexLog("About to call Page2: " + LangPath + "EDL0290_ln_cal.jsp");
						callPage(LangPath + "EDL0290_ln_cal.jsp", req, res);	
					}
					catch (Exception e) {
						flexLog("Exception calling page " + e);
...
protected void procReqCalcInt(MessageContext mc, ESS0030DSMessage user, HttpServletRequest req, HttpServletResponse res, HttpSession ses)
	...
	try
	{
		...
		if (newmessage.getFormatName().equals("EDL029001")) {
			...
			flexLog("Putting java beans into the session");
			ses.setAttribute("error", msgError);
			ses.setAttribute("calcLoans", msgCalc);
			
 
			if (IsNotError) {  // There are no errors
				try {
					flexLog("About to call Page3: " + LangPath + "EDL0290_ln_cal.jsp");
					callPage(LangPath + "EDL0290_ln_cal.jsp", req, res);	
				}
				catch (Exception e) {
					flexLog("Exception calling page " + e);
				...
				try {
					flexLog("About to call Page4: " + LangPath + "EDL0160_ln_balances.jsp");
