...
<h3 align="center">Devolución de Cheques<img src="<%=request.getContextPath()%>/images/e_ibs.gif" align="left" name="EIBS_GIF" ALT="rejection_chk_nsf_list.jsp,EDD0924"></h3>
<hr size="4">
...
<TABLE id="mainTable" class="tableinfo" width="100%">
  ...
  %>
		...
		<%--			<input type="HIDDEN" name="CURCHK_<%= accList.getCurrentRow() %>"
				...
				<DIV NOWRAP><%= accList.getRecord(2) %></DIV>
				</a></TD>
				<TD ALIGN=RIGHT NOWRAP>
				<DIV NOWRAP><a
  ...
  %> 
     		<TR> 
              ...
              <TD ALIGN=Left NOWRAP>
				...
				<img name="IMGREM<%=j%><%=k%>" src="<%=request.getContextPath()%>/images/s/info.gif" alt="More Information" align="absmiddle" border="0" onclick="ShowComments(<%=j%>,<%=k%>)"
				...
				<% if (chkList.getRecord(4).equals("R")) out.print("Devuelto"); else out.print("Pagado"); %></a></DIV></TD>
      			...
      			<TD ALIGN=CENTER NOWRAP><div nowrap><a href="javascript:radioClick('RECNUM<%=j%>',<%= k %>)"><% if (chkList.getRecord(7).equals("1")) { out.print("Camara Entrante");}
