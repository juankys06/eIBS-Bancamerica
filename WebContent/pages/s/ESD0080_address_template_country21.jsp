...
<% 
	...
	String stateDiv		= basicData ? "UC8" : "PAR";
	String stateDiv2 	= basicData ? "UC7" : "UC2";
	String country 		= "CTR" ;
	String poBox 		= "POB" ;
	String zipCode 		= "ZPC" ;
	String mailCode 	= "MLC" ;
         ...
         <tr id="<%=color1%>">
          ...
          <tr id="<%=color1%>"> 
           ...
           <tr id="<%=color2%>">
            ...
            <td nowrap="nowrap" width="66%" > 
              <input type="hidden" name="<%="E"+index+stateDiv2%>"  size="6" maxlength="4" 
              	value="<%=mr.getField("E"+index+stateDiv2).getString()%>" <%=READ_ONLY_TAG%> />
              <input type="text" name="<%="D"+index+stateDiv2%>"  size="37" maxlength="35" 
				onKeypress="javascript:document.forms[0].<%="E"+index+stateDiv2%>.value='';"
				onblur="clrfield('<%="D"+index+stateDiv2%>','<%="E"+index+stateDiv2%>');"
              	value="<%=mr.getField("D"+index+stateDiv2).getString()%>" <%=READ_ONLY_TAG%> 
              	<%if(approval){
              		out.print(mr.getField("F"+index+stateDiv2).getString().equals("Y") ? "id=\"txtchanged\"" : "");} %> />
              <% if ( !readOnly ) {%>
              <a href="javascript:Get2FilterCodes('<%="E"+index+stateDiv2%>','<%="D"+index+stateDiv2%>','PE', document.getElementById('<%="E"+index+state%>').value,document.getElementById('<%="E"+index+stateDiv%>').value)">
				<img src="<%=request.getContextPath()%>/images/1b.gif" alt="Ayuda" align="bottom" border="0" style="cursor:hand"/>
	          </a>
				<img src="<%=request.getContextPath()%>/images/Check.gif" alt="mandatory field" align="bottom" border="0"/> 
              <% } %>
          ...
          <tr id="<%=color1%>"> 
            ...
            <td nowrap="nowrap" width="66%" > 
              <input type="text" name="<%="E"+index+zipCode%>"  size="12" maxlength="11" 
