...
<%
				...
				int posIni = 0;
                 ...
                 try {
               	...
               	try
               	{
					...
					/* Esto se implemento para Banesco Panama.
					 en el campo viene el codigo del pais al cual pertenece el cliente
					 y un flag indicando si quiere consultar los tipos de documento
					 para persona natural o juridica (tabla 34).
					 Si el cliente pertenece a panama, el flag que indica que tipo de
					 documento quiere ver se asigna en el mensaje al campo EWDF02
					 El codigo de pais para panama es 07
					 e.g. codeaux="03_2", codigo de pais = 03, flag = 2
					*/
					
					if( request.getParameter("codeaux") != null &&
						!request.getParameter("codeaux").equals("") &&
						request.getParameter("codeaux").toString().substring(0,2).equals("07")) {
						msgHelp.setEWDF02( request.getParameter("codeaux").toString().substring(3)) ;
					}
	 ...
	 <TABLE  class="tbenter" WIDTH="88%" ALIGN=CENTER>
      ...
      <TD WIDTH="50%" ALIGN=LEFT height="25"> <%
        if ( posIni > 30 ) {
      			int pos = posIni - 31;
      			   out.print("<A HREF=\""+request.getContextPath()+"/pages/s/EWD0002_helpfile_CNOFC.jsp?codeflag=" + codeflag + "&FromRecord=" + pos + "&SelNew=" + selNew + "&SelOld=" + selOld + "\" > <img src=\""+request.getContextPath()+"/images/s/previous_records.gif\" border=0></A>");
        } %>
      </TD>
 	  <TD WIDTH="50%" ALIGN=RIGHT height="25"> <%       
        if (marker.equals("+")) {
      			int pos = posEnd;
      			out.print("<A HREF=\""+request.getContextPath()+"/pages/s/EWD0002_helpfile_CNOFC.jsp?codeflag=" + codeflag + "&FromRecord=" + pos + "&SelNew=" + selNew + "&SelOld=" + selOld + "\" ><img src=\""+request.getContextPath()+"/images/s/next_records.gif\" border=0></A>");
